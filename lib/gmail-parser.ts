...
function extractConfirmationCode(text: string): string | undefined {
  ...
  const patterns = [
    ...
    // Code Airbnb natif: "HM" suivi de lettres+chiffres (ex: HM1234567890, HMABCD123)
...
function extractPropertyName(text: string, subject?: string): string | undefined {
  ...
  if (subject && (
    ...
    /^(?:\[[^\]]+\]\s*)?[A-ZÀ-ŸŒÆ][a-zàâéèêëîïôùûüÿœæ]+\s+(a\s+r[eé]serv|annul|modifi|laiss|r[eé]dig|souhait)/i.test(subject) ||
  ...
  if (subject) {
    ...
    const isPersonSubject = /^(?:\[[^\]]+\]\s*)?[A-ZÀÂÄÉÈÊËÎÏÔÙÛÜŸŒÆ][a-zàâéèêëîïôùûüÿœæ]+(?:\s+[A-Za-zÀ-ÿ\-]+){0,3}\s+(a\s+r[eé]serv|annul|modifi|laiss|part\s|arrive|r[eé]dig|souhait|veut|aimer)/i.test(subject)
      || /\barrive\s+(le|demain|aujourd|dans\s+\d|ce|lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)/i.test(subject)
